...
const Lightfall = ({
  ...
  // Compute finalColors from colors array or individual color1-8 props
  const finalColors = React.useMemo(() => {
    const individual = [color1, color2, color3, color4, color5, color6, color7, color8].filter(Boolean) as string[];
    return individual.length > 0 ? individual : colors;
  ...
  useEffect(() => {
    ...
    const { arr, count, avg } = prepColors(finalColors);
